...
export type WeatherType = null | {
  ...
  localTime: { epochTime: number; timeZone: string };
  currentWeather: CurrentWeatherInterface;
  hourlyForecasts: ForecastHourInterface[];
  dailyForecasts: ForecastDayInterface[];
};

export interface WeatherApiResInterface {
  location: {
    name: string;
    region: string;
    country: string;
...
interface ForecastDayInterface {
  date: string;
  date_epoch: number;
  day: {
    maxtemp_c: number;
  ...
  hour: ForecastHourInterface[];
}

interface ForecastHourInterface {
  time_epoch: number;
  time: string;
  temp_c: number;
  temp_f: number;
